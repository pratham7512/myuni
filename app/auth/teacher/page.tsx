...
export default function TeacherSignInPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const res = await signIn("credentials", { email, password, redirect: true, callbackUrl: "/teacher" })
      ...
      <Card className="bg-card/40 rounded-none border border-white/15">
        ...
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground text-center">No account? <a className="text-primary underline" href="/auth/teacher/signup">Create one</a></p>
          </form>
        </CardContent>
      </Card>
    </div>
