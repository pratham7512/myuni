    ...
    <DashboardShell>
      ...
      {status === "approved" ? (
        <div className="space-y-4">
          ...
          <a href="/teacher/classrooms" className="inline-flex items-center justify-center whitespace-nowrap rounded-none text-sm font-semibold bg-gradient-to-r from-blue-600 to-blue-500 text-white hover:from-blue-500 hover:to-blue-400 shadow-[0_8px_24px_-12px_rgba(37,99,235,0.6)] h-10 px-4 py-2">Manage Classrooms</a>
          <Section title="Your classrooms">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(await prisma.classrooms.findMany({ where: { teacher_id: session.user.id }, orderBy: { created_at: "desc" } })).map(c => (
                <a key={c.id} href={`/teacher/classrooms/${c.id}`} className="rounded-none border border-white/15 bg-card/40 hover:border-primary">
