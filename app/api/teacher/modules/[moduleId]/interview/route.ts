...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ moduleId: string }> }) {
  ...
  const ok = await ensureTeacherOwnsModule(moduleId, session.user.id)
  if (ok.status !== 200) return NextResponse.json({ error: ok.error }, { status: ok.status })
  const interview = await prisma.interviews.findUnique({ where: { module_id: moduleId } })
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ moduleId: string }> }) {
  try {
    ...
    const saved = await prisma.interviews.upsert({
      ...
      update: { description },
      create: { module_id: moduleId, description },
    })
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ moduleId: string }> }) {
  try {
    ...
    const ok = await ensureTeacherOwnsModule(moduleId, session.user.id)
    if (ok.status !== 200) return NextResponse.json({ error: ok.error }, { status: ok.status })
    const existing = await prisma.interviews.findUnique({ where: { module_id: moduleId } })
